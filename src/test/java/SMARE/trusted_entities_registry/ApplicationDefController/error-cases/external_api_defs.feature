Feature: Error Cases - External API Definitions API
  ...
  Scenario: Test HTTP 400 Bad Request - Invalid UUID Format
    ...
    * header x-tenant-name = 'DemoTenant'
    Given path '/applications/invalid-uuid-format/externalApiDefs'
    When method get
    Then status 400
  ...
  Scenario: Test HTTP 401 Unauthorized - Missing Token
    ...
    * header Content-Type = 'application/json'
    * header x-tenant-name = 'DemoTenant'
    Given path '/applications/test-app-id/externalApiDefs'
    When method get
    Then status 401
  ...
  Scenario: Test HTTP 403 Forbidden
    ...
    * def accessToken = response.accessToken
    ...
    And path '/applications/test-app-id/externalApiDefs'
  ...
  Scenario: Test HTTP 404 Not Found - Invalid Application ID
    ...
    * header Content-Type = 'application/json'
    * header x-tenant-name = 'DemoTenant'
    Given path '/applications/1111111-1111-1111-1111-111111111999/externalApiDefs'
    When method get
    Then status 404

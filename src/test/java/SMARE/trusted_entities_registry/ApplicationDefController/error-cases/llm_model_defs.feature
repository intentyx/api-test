Feature: Error Cases - LLM Model Definitions API
  ...
  Scenario: Test HTTP 400 Bad Request - Invalid UUID Format
    ...
    * header x-tenant-name = 'DemoTenant'
    Given path '/applications/invalid-uuid-format/llmModelDefs'
    When method get
    Then status 400
  ...
  Scenario: Test HTTP 401 Unauthorized - Missing Token
    ...
    * header Content-Type = 'application/json'
    * header x-tenant-name = 'DemoTenant'
    Given path '/applications/test-app-id/llmModelDefs'
    When method get
    Then status 401
  ...
  Scenario: Test HTTP 403 Forbidden
    ...
    And header x-tenant-name = 'DemoTeanant'
    ...
    * def accessToken = response.accessToken
    ...
    And path '/applications/test-app-id/llmModelDefs'
  ...
  Scenario: Test HTTP 404 Not Found - Invalid Application ID
    ...
    * header Content-Type = 'application/json'
    * header x-tenant-name = 'DemoTenant'
    Given path '/applications/1111111-1111-1111-1111-111111111999/llmModelDefs'
    When method get
    Then status 404
    * print "Not Found Response:", response

  Scenario: Test HTTP 405 Method not allowed - Invalid Request Body for Create
    * url 'http://localhost:8080/smare'
    * def loginResult = call read('classpath:SMARE/smare_login.feature')
    * def accessToken = loginResult.accessToken
    * header Authorization = 'Bearer ' + accessToken
